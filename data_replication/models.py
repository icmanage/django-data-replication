...
class ReplicationTracker(models.Model):
    ...
    def __str__(self):
        return "%r replication of %r" % (
            self.get_replication_type_display(),
    ...
    def get_replicator(self, **kwargs):
        ...
        try:
            ...
            try:
                module = importlib.import_module(
                    "{}.replication".format(self.content_type.app_label)
                )
            except ImportError:  # pragma no cover
                raise ImportError(
                    "Unable to find replication.py in app %s" % self.content_type.app_label
                )
        ...
        if module:
            for name in dir(module):
                ...
                if (
                    inspect.isclass(Replcate)
                    and Replcate not in ignored_classes
                    and issubclass(Replcate, target_module)
                ):
                    if ContentType.objects.get_for_model(Replcate.model) == self.content_type:
                        if replication_class_name is not None:
                            if replication_class_name == Replcate().__class__.__name__:
                                log.debug(
                                    "Using matching replication with class name %s - %r",
        ...
        if len(options) == 1:
            ...
            raise IOError(
                "Unable to identify replication module for %s many found use replication_class_name %r"
                % (self.content_type.app_label, options)
            )
