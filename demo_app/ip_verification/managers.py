...
COVERAGE_KEY_NAMES = {"cond_expr": "Cond Expression", "fsm": "FSM"}
...
class TestResultLinkQuerySet(models.query.QuerySet):
    def _as_json(self, pretty=False, as_list=False):
        data = []
        _data = self.filter().values_list(
            "pk", "options", "summary_id", "last_used", "updated_status__name"
        )
        ...
        if pretty:
            ...
            return json.dumps(
                data, sort_keys=False, default=json_serial, indent=4 if pretty else None
            )
        return data
    ...
    def get_summary_data(self, data=None):  # noqa: C901
        ...
        try:
            launch_time = min(
                [extract_time_from_json(x.get("launch_time")) for x in data if x.get("launch_time")]
            )
        ...
        if launch_time and end_time:
            ...
            result["elapsed_secs_display"] = ip_verification_app.humanize_time(
                (end_time - launch_time).total_seconds()
            )
    ...
    def get_expanded_data(self, data=None, unique_keys=None):
        ...
        non_unique_keys = [
            x for x in unique_keys if x not in ["test_case", "test_instance", "testcasename"]
        ]

        grouping = [
            x for x in non_unique_keys if x in ["sub_project", "suite_name", "subsuite_name"]
        ]
        if not grouping:
            grouping = ["suite_name"]
        ...
        for item in data:
            ...
            if not object_data.get(least_key):
                ...
                search = [
                    "%s=%s" % (LEAST_KEY_MAP.get(key, key), least_key[idx])
                    for idx, key in enumerate(grouping)
                ]
        ...
        result = {
            "key_name": [
                LEAST_KEY_MAP.get(x, x) for x in grouping
            ],  # No longer needed here for legacy
        ...
        for key, values in object_data.items():
            ...
            result["{}".format(_values[0])] = {
                k: v for k, v in object_data[key].items() if k != "ids"
            }
    ...
    def get_coverage_data(self, data=None, unique_keys=None):  # noqa: C901
        ...
        for item in data:
            ...
            for k, v in coverage.items():
                if (
                    k.endswith("_numerator")
                    or k.endswith("_denominator")
                    or k.endswith("_nume")
                    or k.endswith("_deno")
                ):
                    continue
                ...
                if coverage.get("{}_numerator".format(k)) and coverage.get(
                    "{}_denominator".format(k)
                ):
                    ...
                    # value_str = "{:.2%}".format(value)
                    value_str += " ({}/{})".format(
                        coverage.get("{}_nume".format(k)), coverage.get("{}_deno".format(k))
                    )
                    try:
    ...
    def get_planned_test_data(self, data=None):
        ...
        for item in data:
            ...
            group_name = (
                item.get("regression_tag"),
                item.get("product"),
                item.get("family_name"),
                item.get("subfamily_name"),
            )
            ...
            ptcs = ptcs.values_list(
                "pk",
                "regression_tag__name",
                "product__name",
                "family__name",
                "sub_family__name",
                "plan_date",
            ...
            prior_ptcs = prior_ptcs.values_list(
                "pk",
                "regression_tag__name",
                "product__name",
                "family__name",
        ...
        return {
            ...
            "planned_test_cases": [ptc for v in ptc_counter.values() for ptc in v[1:]],
            "prior_test_plans": [
                ptc
                for v in prior_ptc_counter.values()
                for ptc in v
                if ptc["pk"] not in current_plan_pks
            ],
        }
...
class TestResultLinkManager(models.Manager):
    ...
    def filter_latest_results_for_run(self, regression_tag, user, run, summary=None):
        ...
        if not current and not summary:
            log.debug(
                "Invalid Summary listing for  Regression ID: %r  Run ID: %r",
                regression_tag.id,
                run.id,
            )
        ...
        if stale:
            ...
            log.debug(
                "Removed %d stale summary data for Regression ID: %r  Run ID: %r",
                len(stale),
                regression_tag.id,
                run.id,
            )
        ...
        if missing:
            ...
            log.debug(
                "Added %d missing summary data for Regression ID: %r  Run ID: %r",
                len(missing),
                regression_tag.id,
                run.id,
            )
...
class RegressionTagSummaryQuerySet(models.query.QuerySet):
    def filter_by_user(self, user):
        ...
        return self.filter(Q(user_deleted=False) | Q(user__name=user.username), available=True)

    def complete_jobs(self):
        prior_date = now() - datetime.timedelta(days=14)
        return self.filter(
            Q(finalized=True)
            | Q(Q(passing_count__gt=0) | Q(failing_count__gt=0), finalized=False, untested_count=0)
            | Q(last_updated__lte=prior_date),
            available=True,
        )
